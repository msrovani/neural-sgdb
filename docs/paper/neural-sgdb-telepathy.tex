...
% Authors: replace the placeholder \author with the real list before submit.
...
\author{M.~Srovani\thanks{Project: \url{https://github.com/msrovani/neural-sgdb}}
...
two-instance ``telepathy'' convergence demo, and the honest costs of the
model: eventual consistency, absence of global ordering, and deferred conflict
resolution.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A storage layer (CRC append-log and a byte-exact log codec) with
        ...
        invalidation.
  \item A conflict-preserving CRDT synchronization protocol (``telepathy'')
...
\subsection{Synchronization (``telepathy'')}
\label{sec:sync}
...
(\S\ref{sec:sync}) ships only versions a peer has not seen.

Exchanging versions is the causal trigger; transferring documents is a
diff-pull: a peer that learns another advanced replicates the missing docs
(\texttt{get} $\to$ \texttt{put}). Two instances converge with no central
server. Because resolution writes are themselves causal CRDT writes,
arbitration outcomes propagate to every peer.
...
\subsection{Telepathy convergence}
\label{sec:eval-telepathy}

The \texttt{p2p\_telepathy} experiment runs two \texttt{Sgdb} instances over an
...
--example p2p\_telepathy --features p2p} (the telepathy experiment).
