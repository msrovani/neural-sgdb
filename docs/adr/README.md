...
| 0009 | Index snapshot on open; metrics-gated auto-adapt | Storage = truth; TKCK-like index snapshot; measure `open_ms`/`doc_count`/reopens; auto persist+fast-mount when budget breaks — **not** mid-query; §3 fast-mount IDX1 implementado (**v1.1.29**, seam `NEURAL_SGDB_INDEX_SNAPSHOT`); §5 auto-adapt ainda ROADMAP |
