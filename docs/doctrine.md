...
Rules:
1. Same embedding model AND dimension on remember and recall. Demo trigram is NOT semantic — match the same words, or pass a real `embedding`. New dim on a live corpus → call era_report; do not force the write (BQ would truncate).
...
MCP lists 4 tools: remember, recall, health, curate. health(view=era) is era_report. curate(op=...) covers explain/reinforce/graph. Legacy tool names still work if a client calls them.
