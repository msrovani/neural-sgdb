...
| `sgdb.rs` | `Sgdb` — public facade: remember/recall/rag_context/checkpoint; `Hit` (tipado v1.1.6); recall modes/temporal/entities/scoped; seams de conteúdo | Facade (port of layers.rs) |
| `embedder.rs` | **v1.1.2**: `Embedder` trait + `DemoEmbedder` (trigram hash) — zero-dep seam p/ modelo real | Seam (provider declares) |
