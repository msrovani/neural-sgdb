...
Status: **v1.1.x maintenance line (crate v1.1.15)** —
stable API, zero deps, `no_std` + `std`, CI gates green. Crate version
**1.1.15**; histórico v1.1.2–v1.1.15 no `CHANGELOG.md`. This roadmap is honest
...
🔜 **Agentic substrate (north star):**
  o produto é o agente **usando** o DB — duas passadas (gather evidence,
  depois remember), doutrina MCP (`nsgdb://doctrine` / `nsgdb://session`),
  hits tipados `format=json`, `examples/agent_protocol.rs` (23),
  `two_ai_protocol.rs` (16), `memory_arena_eval.rs`. Não hoarding.
...
- **No Chrome Web Store / browser auto-capture as product** — `extension/`
  is parked (stub). Agents speak MCP; the core does not scrape tabs.