...
**v1.1.26** — substrate **agêntico** (MCP 5 tools contract **1.1.28**, doutrina,
...
Extensão de browser / Store **estacionadas**. Crate em `Cargo.toml`: **1.1.28**. 
...
SemVer; `crates/nsgdb-embed` (LocalEmbedder 384-dim, `cargo run --manifest-path crates/nsgdb-embed/Cargo.toml --example demo`) and `crates/nsgdb-wasm` (`Storage` stub) are host crates — core SemVer = `Cargo.toml` (**1.1.28**).
